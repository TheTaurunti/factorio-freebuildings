...
function Breakdown_Recipe_Ingredients(recipe)
  ...
  while (needs_processing and iterations_done < max_iterations) do
    ...
    for _, ingred in ipairs(working_ingredients) do
      ...
      elseif (Items_To_Skip_Breakdown[ingred.name])
      then
        item_inputs[ingred.name] = ingred.amount
      else
        local breakdown_ingreds = Item_Made_Free_Ingredients[ingred.name]
